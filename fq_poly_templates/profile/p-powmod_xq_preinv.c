...
main(int argc, char** argv)
{
    ...
    for (n = 0; n < ncases; n++)
    {
        ...
        */
        {
            TEMPLATE(T, poly_randtest)(f, state, lenf, ctx);
    ...
    loop:
        ...
        {
            __TEMPLATE(T, poly_powmod_xq_preinv_direct)(xq, f, finv, ctx);
        }
        prof_stop();
        t[0] += get_clock(0);
        ...
        for (lo = 0; lo < loops; lo++)
        {
            __TEMPLATE(T, poly_powmod_xq_preinv_compose)(xq, f, finv, ctx);
        }
        prof_stop();
        t[1] += get_clock(0);
