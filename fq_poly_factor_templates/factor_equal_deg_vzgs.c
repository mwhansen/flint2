...
/* Algorithm from "Computing Frobenius Maps and Factoring Polynomials"
 * by von zur Gathen and Shoup, 1992. */
                                         ...
                                         const TEMPLATE(T, ctx_t) ctx)
{
    ...
    else
    {
        ...
        while (!factors_found)
        {
            factors_found = TEMPLATE(T, poly_factor_equal_deg_prob_vzgs)
                                (f, g, state, pol, d, ctx);
        }
        ...
        TEMPLATE(T, poly_factor_equal_deg_vzgs) (factors, f, d, ctx);
        if (factors_found == 2)
        {
            TEMPLATE(T, poly_divrem) (h, r, h, g, ctx);
            TEMPLATE(T, poly_factor_equal_deg_vzgs) (factors, g, d, ctx);
        }
        if (!TEMPLATE(T, poly_is_one)(h, ctx))
            TEMPLATE(T, poly_factor_equal_deg_vzgs) (factors, h, d, ctx);
