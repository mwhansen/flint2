                                       ...
                                       const TEMPLATE(T, ctx_t) ctx)
{
    ...
    else
    {
        ...
        while (!factors_found)
        {
            factors_found = TEMPLATE(T, poly_factor_equal_deg_prob_cz)
                                (f, g, state, pol, d, ctx);
        }
        ...
        TEMPLATE(T, poly_factor_equal_deg_cz) (factors, f, d, ctx);
        if (factors_found == 2)
        {
            TEMPLATE(T, poly_divrem) (h, r, h, g, ctx);
            TEMPLATE(T, poly_factor_equal_deg_cz) (factors, g, d, ctx);
        }
        if (!TEMPLATE(T, poly_is_one)(h, ctx))
            TEMPLATE(T, poly_factor_equal_deg_cz) (factors, h, d, ctx);
