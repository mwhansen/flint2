...
int
TEMPLATE(T, poly_factor_equal_deg_prob_cz) (TEMPLATE(T, poly_t) f,
                                            TEMPLATE(T, poly_t) g,
                                            flint_rand_t state,
                                            ...
                                            const TEMPLATE(T, ctx_t) ctx)
{
    ...
    if (pol->length <= 1)
    {
        TEMPLATE_PRINTF("Exception (%s_poly_factor_equal_deg_prob_cz): \n", T);
    ...
    TEMPLATE(T, poly_gcd) (f, a, pol, ctx);
    ...
    TEMPLATE(T, poly_gcd) (f, b, pol, ctx);
